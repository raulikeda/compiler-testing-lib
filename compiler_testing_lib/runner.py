...
class TestRunner:
    ...
    def run_tests(self, command_template, asm_build_template=None, asm_run_template=None, check=None, ebnf_check=None,
                  build_template=None, run_template=None, target_extension=None, native_errors=False):
        # Generalized two-stage grading (student compiler emits a target-language
        # file which is then built and run): `build_template`/`run_template`
        # supersede the v3.0-specific `asm_*` names, which remain as aliases.
        # `target_extension` is the extension of the emitted file (default 'asm'
        # for the v3.0 flow). `native_errors=True` grades error tests by the
        # target toolchain's native failure (non-zero exit; for tests marked
        # error_phase: parse, also no stdout) instead of the [Lexer]/[Parser]/
        # [Semantic] tag convention -- used with corpora generated by
        # compiler_testing_lib.transpiler.
        build_template = build_template or asm_build_template
        run_template = run_template or asm_run_template
        self._target_extension = target_extension or 'asm'
        self._two_stage = (self.version in ['v3.0', 'x3.0']
                           or build_template is not None
                           or run_template is not None
                           or target_extension is not None)
        self._native_errors = native_errors
        asm_build_template, asm_run_template = build_template, run_template
        divergences = []
        issue = []
        ...
        if len(issue) == 0:
            ...
            for idx, test in enumerate(self.tests):
                ...
                try:
                    input_values = ('\n').join(test['input'])
                    # Two-stage flow: compile first (no stdin), then build and run the emitted file with stdin
                    if self._two_stage:
                        ...
                        if exit_code == 0 and expect_success:
                            ...
                            asm_file = f"{base}.{self._target_extension}"
                            ...
                            if asm_build_template:
                                build_cmd = asm_build_template.format(asm_file=asm_file, src_file=asm_file, exe_file=exe_file, base_name=base_name, dir=dir_name)
                            else:
                            ...
                            # Run command
                            if asm_run_template:
                                run_cmd = asm_run_template.format(exe_file=exe_file, src_file=asm_file, base_name=base_name, dir=dir_name)
                            else:
                                run_cmd = exe_file
                ...
                if expect_success:
                    ...
                    # Native-error grading (transpiler-generated corpora): the
                    # failure is the target toolchain's own; no tag convention.
                    if self._native_errors:
                        if test.get('error_phase') == 'parse' and stdout.strip():
                            divergences.append({
                                'index': test.get('index', idx+1),
                                'description': test['description'],
                                'code': code,
                                'input': input_values,
                                'expected': 'A parse-phase failure with no output',
                                'actual': f'Output before failing: {stdout}'
                            })
                            if len(divergences) >= self.max_errors:
                                break
                        continue
